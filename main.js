...
video.addEventListener('mousedown', (ev) => {
    ...
    seekstartmousex = ev.clientX;
    video.requestPointerLock();
});
video.addEventListener('mouseup', (_) => {
    seeking = false;
...
window.addEventListener('keydown', (ev) => {
    ...
    if (ev.key == 'ArrowLeft') {
        ...
        startNext(Math.max(videoId - 1, 0));
        speedMultip = 0;
        video.playbackRate = getSpeedMultip();
        return false;
    }
...
function intvMain() {
    ...
    } else if (videoEndTime - video.currentTime <= transition_time_s * getSpeedMultip() + 0.1) {
...
videoEndTime = -1;
function startNext(nextVideo = videoId + 1) {
    if (!is_loop.checked) {
        // ループじゃなかったら次の曲
        videoId = nextVideo;
        speedMultip = 0;
